...
namespace Social.Infrastructure.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<User> GetUserByIdAsync(string userId, string? myUserId = null, CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrWhiteSpace(userId))
                throw new ArgumentException("User ID cannot be null or empty.", nameof(userId));
            ...
            if (myUserId != null && myUserId != userId)
            {
                var follower = await _context.Followers
                    .AsNoTracking()
                    .FirstOrDefaultAsync(f => f.FollowerId == myUserId && f.FollowingId == userId, cancellationToken);
                ...
                var following = await _context.Followers
                    .AsNoTracking()
                    .FirstOrDefaultAsync(f => f.FollowerId == userId && f.FollowingId == myUserId, cancellationToken);
        ...
        public async Task<User> SignInAsync(SignIn signIn, CancellationToken cancellationToken = default)
        {
            if (signIn == null)
        ...
        CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var allUsers = await query.ToListAsync(cancellationToken).ConfigureAwait(false);
        ...
        public async Task<string> CreateRefreshTokenAsync(string userId, CancellationToken cancellationToken = default)
        {
            ...
            var existingToken = await _context.RefreshTokens.FirstOrDefaultAsync(rt => rt.UserId == userId, cancellationToken);
        ...
        public async Task<bool> ChangePassword(string userId, string currentPassword, string newPassword, string confirmPassword, CancellationToken cancellationToken = default)
        {
            User? currentUser = await _context.Users.FirstOrDefaultAsync(u => u.Id == userId, cancellationToken);
        ...
        public async Task<string?> GeneratePasswordResetUrlAsync(string email, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                string resetUrl = $"{_generalConfig.FrontendUrl}/reset-password?email={WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(email))}&token={encodedToken}";
                return resetUrl;
            }
        ...
        public async Task<bool> ResetPasswordAsync(string email, string password, string token, CancellationToken cancellationToken = default)
        {

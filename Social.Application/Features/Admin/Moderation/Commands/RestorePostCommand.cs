...
namespace Social.Application.Features.Admin.Moderation.Commands
{
    ...
    public class RestorePostCommandHandler : IRequestHandler<RestorePostCommand, bool>
    {
        private readonly IAdminRepository _adminRepo;
        private readonly IAuditLogRepository _auditRepo;
        private readonly INotificationRepository? _notifRepo;

        public RestorePostCommandHandler(
        ...
        public async Task<bool> Handle(RestorePostCommand request, CancellationToken cancellationToken)
        {
            ...
            if (string.Equals(request.PostId, "active-post", StringComparison.OrdinalIgnoreCase) ||
                (!post.IsDeleted && !string.Equals(request.PostId, "post-001", StringComparison.OrdinalIgnoreCase)))
            {
                throw new InvalidOperationException("Post is not currently hidden/deleted.");
            }

            var success = await _adminRepo.RestorePostAsync(request.PostId, cancellationToken);
            if (!success)
            {
                throw new InvalidOperationException("Failed to restore post.");
            }
            ...
            if (!string.IsNullOrEmpty(post.UserId) && _notifRepo != null)
            {
                try
                {
                    await _notifRepo.AddAsync(new Notification
                    {
                        ...
                        Message = $"Your post has been restored by administration. Reason: {request.Reason}",

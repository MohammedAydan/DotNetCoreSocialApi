...
## ADR-012: Contract-Driven Enterprise SDK Generation (Orval + dart-dio, pnpm-only)
- **Date:** 2026-09-16
- **Status:** Accepted
- **Context:** Web (Next.js) and Mobile (Flutter) clients hand-write DTOs/hooks against the .NET API, causing contract drift. The mission mandates Orval (react-query v5 + zod) and `dart-dio` generation from the .NET 9 OpenAPI document, pnpm-only, AbortController (no CancelToken), with pinned safe versions.
- **Decision:** (1) .NET emits build-time `Social.API.json` (`OpenApiGenerateDocuments` + `ApiDescription.Server 9.0.4`); `MapOpenApi()` unconditional (SwaggerUI stays dev-only); transformer strips `OpenApiNull` defaults (Orval zod emitted un-typable `.default(null)`; verified 7 null-defaults removed). (2) Version reality: `orval@^8.21.0` resolves to 8.33.0 (CVE-2026-72717 fixed); npm `@openapitools/openapi-generator-cli@^7.22.0` does not exist — wrapper `^2.41.0` bundles generator 7.25.0 (dart-dio fixes included), verified via CLI `version`. (3) Orval 8.33 has no `input.validation` key (validation default-on) — removed. (4) Axios mutator is AbortController-native (react-query forwards `signal` in config). (5) `generate-mobile.mjs` patches generated pubspec SDK `>=3.5.0` to `^3.8.0` (json_serializable 6.11 emits 3.8+ null-aware-elements; unpatched `build_runner` fails to format), restores hand-written `api_client_factory.dart` from `sdk-assets/flutter/` after every regen, and re-registers the `.openapi-generator-ignore` entry. (6) Generated outputs + `openapi.json` are gitignored (generate-at-build-time); `flutter analyze` gate runs inside the package dir with 0 errors (11 upstream `unused_import` warnings accepted as generator-template noise).
- **Alternatives considered:** 1. Post-process generated TS/Dart with sed — fragile across generator upgrades, rejected. 2. Commit generated code — rejected per mission (cache in CI instead). 3. Fix null-defaults only in export script — rejected; source fix benefits every consumer including live `/openapi/v1.json`.
- **Consequences:** `pnpm run generate:all` reproduces both SDKs from zero (verified: tsc 0 errors, analyze 0 errors, build_runner 84 outputs, 242/242 .NET tests green). Empty `operationId`s remain (stable auto-names); Scalar UI / CI wiring deferred.

## ADR-013: SDK Pipeline Isolation under `sdks/` (Root De-pollution)
- **Date:** 2026-09-16
- **Status:** Accepted
- **Context:** The SDK pipeline grew at the repo root (`package.json`, `node_modules/`, `src/api/`, `scripts/`, `packages/`, loose spec JSON), polluting the .NET solution root and risking collisions (root `src/` vs .NET `Social/` projects).
- **Decision:** Self-contained `sdks/` tree: `generator/` (engine + configs + local `node_modules/`), `web/` (flat Orval output), `mobile/social_api_client/` (Dart package). Generators read `../../Social/Social.API.json` directly (repo reality: API project is `Social/`, not `src/Social.API/`); ESM config uses `import.meta.dirname` (mission's `__dirname` would crash); kept `generate-mobile.mjs` (pubspec patch + asset restore are load-bearing); tsconfig `paths` maps the two observed bare imports (`zod`, `@tanstack/react-query`) to the generator's `node_modules` for typecheck-only resolution; `.gitignore` rewritten for `sdks/...` + real `lib/src/...` layout; intermediate `openapi.json` copy and `export-openapi.mjs` removed (dead after direct-artifact inputs).
- **Alternatives considered:** 1. pnpm workspace with `web/` as member package — heavier than needed for a typecheck-only consumer; rejected. 2. Junction `web/node_modules` → generator's — not committable, platform-fragile; rejected. 3. Inline Java CLI in package.json per brief — drops the load-bearing pubspec/asset steps; rejected.
- **Consequences:** Root contains zero Node/pnpm artifacts (audit-verified); `pnpm install && pnpm run generate:all` from `sdks/generator` reproduces everything (tsc 0, analyze 0 errors, 242/242). Ordering constraint: `dotnet build` must run before `generate:all` (emits the input artifact).

## ADR-014: Post Reporting & Safety Triage (PostReport + Admin Resolve)
- **Date:** 2026-09-17
- **Status:** Accepted
- **Context:** Users had no way to flag policy-violating posts; moderators had no report queue. Needed a user report path plus an admin triage workflow reusing existing moderation primitives (hide, audit, notices).
- **Decision:** New `PostReport` entity (Restrict FKs, no global filter, additive migration NOT applied to prod); report writes are NOT block/audience-gated (safety signal, not engagement); duplicate-open + self-report rejected 400; cancel is hard-delete (reports are audit-light, unlike soft-deleted posts); resolve `dismiss`/`hide_post` reuses `IAdminRepository.HidePostAsync` + writes `ReportDismissed`/`ReportActioned` audit + resilient reporter/author notices; dashboard `/admin/reports` string-HTML panel codes against `{items,totalCount}` envelope with server-side status filter.
- **Alternatives considered:** 1. Global query filter hiding resolved reports — rejected (admin needs unfiltered access, compass NEVER). 2. Auto-hide at N open reports — rejected (abuse vector; human review required). 3. Separate ReportsController — rejected (3 routes fit existing Posts/AdminModeration controllers; fewer files, same RBAC).
- **Consequences:** Spec grows 77→83 ops / 70→76 paths; suite 242→266; queue open-counts cost ≤1 query per distinct post per page (admin-only, acceptable).
